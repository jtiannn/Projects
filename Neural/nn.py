...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        for parent in parents:
            inputs.append(self.nodes[parent])
    ...
    def backprop(self):
        ...
        inputs = self.get_inputs(loss_node)
        gradient = loss_node.backward(inputs, 1)
        self.gradients[loss_node] = 1.0
        parents = loss_node.get_parents()
        l = 0
        for parent in parents:
            self.gradients[parent] = self.gradients[parent] + gradient[l]
            l = l + 1
       
        i = 2
        while i <= len(self.nodesList):
            node = self.get_nodes()[-i]
            inputs = self.get_inputs(node)
            grad = self.gradients[node]
            gradient = node.backward(inputs, grad)
            parents = node.get_parents()
            #update here for gradient shape list
            k = 0
            for parent in parents:
                self.gradients[parent] = self.gradients[parent] + gradient[k]
                k = k + 1
            i = i + 1
    ...
    def step(self, step_size):
        ...
        for var in self.variables:
            var.data = var.data - (self.gradients[var] * step_size)
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [gradient.dot(np.transpose(inputs[1])), np.transpose(inputs[0]).dot(gradient)]
...
class MatrixVectorAdd(FunctionNode):
    ...
    @staticmethod
    def backward(inputs, gradient):
        #find a way to make the return value of x be the size of vector x
...
class ReLU(FunctionNode):
    ...
    @staticmethod
    def backward(inputs, gradient):
        new = np.maximum(np.zeros_like(inputs[0]), inputs)
        return [np.where(new[0] == 0, 0, gradient)]
...
class SquareLoss(FunctionNode):
    ...
    def forward(inputs):
        "*** YOUR CODE HERE ***"
        c = np.subtract(inputs[0], inputs[1])
        c = np.square(c)
        c = c * .5
        return np.mean(c)
    ...
    @staticmethod
    def backward(inputs, gradient):
        return [np.subtract(inputs[0], inputs[1]) * gradient / inputs[0].size, np.subtract(inputs[0], inputs[1]) * gradient / inputs[1].size * -1]
