    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 7
                if self.ant.container == True and insect.container == False and self.ant.ant is None:
                    self.ant.ant = insect
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                ...
                elif self.ant.__class__ is QueenAnt and self.ant.impostor is False:
                    return
                else:
                    self.ant = None
            else:
                if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                    if self.ant.ant.__class__ is QueenAnt and self.ant.ant.impostor is False:
                        return
                    self.ant.ant = None 
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for i in range(self.min_range):
            checking = checking.entrance
        distance = 0
        while checking.entrance.name != 'Hive' and checking.entrance is not None and len(checking.bees) == 0 and distance < self.max_range:
            if len(checking.bees) == 0 or distance < self.min_range:
                checking = checking.entrance
                distance += 1
            else:
                nearest = checking
        return random_or_none(checking.bees) 
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            lst = self.place.bees[:]
            for insect in lst:
                insect.reduce_armor(self.damage)
            self.place.remove_insect(self)
        # END Problem 4A
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        QueenAnt.num += 1
        self.num = QueenAnt.num
        if self.num > 1: #imposter
            self.impostor = True
        # END Problem 9
    ...
    def action(self, colony):
        ...
        if self.impostor is True:
            ...
            while current.exit is not None:
                if current.ant and current.ant not in self.doubled:
                    current.ant.damage *= 2
                    if current.ant.container and current.ant.ant not in self.doubled:
                        current.ant.ant.damage *= 2
                        self.doubled.append(current.ant.ant)
                elif current.ant and current.ant in self.doubled:
                    if current.ant.container and current.ant.ant not in self.doubled:
                        if current.ant.ant:
                            current.ant.ant.damage *= 2
                            self.doubled.append(current.ant.ant)
                self.doubled.append(current.ant)
