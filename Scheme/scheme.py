...
def eval_all(expressions, env):
    ...
    if expressions.second is nil:
        return scheme_eval(expressions.first, env)
    #elif expressions.first is nil:
        #return None
    else:
        #scheme_eval(expressions.first, env)
        eval_all(expressions.second, env)
    """
...
def do_lambda_form(expressions, env):
    ...
    #return Pair('lambda', expressions)
    # END Question 8
...
def do_and_form(expressions, env):
    ...
    # BEGIN Question 14B
    while expressions:
        if expressions.second is nil and expressions.second is not False:
            return scheme_eval(expressions.first, env)
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        # BEGIN Question 15A
        value = scheme_eval(clause.first, env)
...
def check_formals(formals):
    ...
    storage = []
    while formals:
        if formals.first in storage or not scheme_symbolp(formals.first):
            raise SchemeError
        storage.append(formals.first)
        formals = formals.second
